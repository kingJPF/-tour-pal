...
public class ConnectionSQL {
	...
	private String prot = "localhost:3306/";
        ...
        try{  
            ...
            conn=DriverManager.getConnection(  
                  //  "jdbc:mysql://localhost:3306/struts_tutorial","root","1234");  
            		"jdbc:mysql://" + prot + database,user_name,user_password);
    ...
    public boolean Insert_choice(User user){  
        ...
        try{  
            Class.forName("com.mysql.jdbc.Driver");  
            conn=DriverManager.getConnection("jdbc:mysql://" + prot  + database,user_name,user_password);
   ...
   //查找用户
    ...
    public boolean Team_match(User user ,String Leadname) {
        ...
        try{  
            ...
            if (temp == null){
            	...
            	sql = "UPDATE user SET flag = ?, start = ?,end = ?, start_date = ? ,info = ? WHERE userid = ?";
                ...
                ps1.setString(5,Integer.toString(i));
                ps1.setString(6,user.getUsername());
                ps1.executeUpdate();
        		...
        		ps3.setString(1,"1");
        		ps3.setString(2,start);
        		ps3.setString(3,end);
        		ps3.setString(4,start_date);
        		
                ps3.setString(5,temp);
    ...
    public boolean Match_Map(User user,ArrayList<User_info_more> User_mate){
        ...
        try{  
            Class.forName("com.mysql.jdbc.Driver");  
            conn=DriverManager.getConnection("jdbc:mysql://" + prot   + database,user_name,user_password);  
            String sql=  "select * from user where start = ? and end = ?  and flag = 1";  
            ps=conn.prepareStatement(sql);  
            ps.setString(1,user.getStart());  
            ps.setString(2,user.getEnd());
            ResultSet rsl = ps.executeQuery();
    ...
    public boolean Exit(User user){  
        ...
        try{  
            ...
            if(user.getFlag()==0){
            	System.out.println(user.getFlag());
            	String sql = "Select userid from user where info = ? and userid != ?";
            	ps=conn.prepareStatement(sql);
            	ps.setString(1,user.getInfo());
            	...
            	while(rsl.next()){
            		username1 = rsl.getString("userid");
            	}
            	System.out.println("1");
            	sql = "UPDATE user SET flag = 0 WHERE userid = ?";
            	ps1=conn.prepareStatement(sql);
	            ...
	            ps2.setString(5,user.getUsername());
	           ...
	           System.out.println("2");
            	...
            	String sql=  
	                    "UPDATE user SET flag = ?, start = ?,end = ?,start_date = ?,info = ? WHERE userid = ?"; 
            	System.out.println("4");
	            ps=conn.prepareStatement(sql);  
	            ps.setString(1,"0");
	            ps.setString(2,null);
